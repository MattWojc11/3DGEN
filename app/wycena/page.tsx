  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--bg)' }}>
      ...
      <header style={{
        ...
        <div style={{ padding: '0 1.25rem', height: '56px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', minWidth: 0 }}>
            <Link
              href="/"
              style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--text-muted)', textDecoration: 'none', fontFamily: "'Space Mono', monospace", fontSize: '0.75rem', letterSpacing: '0.08em', textTransform: 'uppercase', transition: 'color 0.15s', flexShrink: 0 }}
              onMouseEnter={e => (e.currentTarget.style.color = 'var(--accent)')}
              onMouseLeave={e => (e.currentTarget.style.color = 'var(--text-muted)')}
            >
              <ChevronLeft size={14} /> Powrót
            </Link>
            <div className="hide-mobile" style={{ width: '1px', height: '20px', background: 'var(--border)' }} />
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', minWidth: 0 }}>
              <div style={{ width: '24px', height: '24px', background: 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', clipPath: 'polygon(0 0, 100% 0, 100% 75%, 75% 100%, 0 100%)', flexShrink: 0 }}>
                <Box size={12} color="#000" />
              </div>
              <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '0.85rem', fontWeight: 700, color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                3D<span style={{ color: 'var(--accent)' }}>GEN</span>
                <span className="hide-mobile" style={{ color: 'var(--text-muted)', fontWeight: 400 }}> / Wycena</span>
              </span>
            </div>
          </div>
          ...
          {volume && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', flexShrink: 0 }}>
              <span className="label-tag hide-mobile">Model załadowany</span>
      ...
      {/* ─── Error ──────────────────────────────── */}
      {errorMsg && (
        <div style={{ background: 'rgba(239,68,68,0.08)', borderBottom: '1px solid #7f1d1d', padding: '0.75rem 1.25rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
      ...
      {/* ─── Main Layout ─────────────────────── */}
      <main className="wycena-layout">

        {/* LEFT — 3D Viewer / Upload */}
        <div className="wycena-viewer-col">

          {!fileUrl ? (
            /* Drop Zone */
            <div
              className="wycena-dropzone"
            ...
            >
              ...
              <div style={{
                ...
                border: `2px solid ${isDragging ? 'var(--accent)' : 'var(--border-light)'}`,
              ...
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 'clamp(1rem, 3vw, 1.25rem)', fontWeight: 600, marginBottom: '0.5rem' }}>
                  Przeciągnij plik STL
                </div>
                <div className="label-tag">lub kliknij, aby wybrać · maks. 50 MB</div>
              </div>
            </div>
            ...
            <div style={{ flex: 1, position: 'relative', minHeight: 0, overflow: 'hidden' }}>
              ...
              <div style={{
                ...
                <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '0.7rem', color: 'var(--text-muted)', maxWidth: '160px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {fileName}
                </span>
              </div>

              {isUploading && (
                <div style={{ position: 'absolute', bottom: '1rem', left: '50%', transform: 'translateX(-50%)', background: 'rgba(10,10,10,0.9)', border: '1px solid var(--accent)', padding: '0.625rem 1.25rem', display: 'flex', alignItems: 'center', gap: '0.75rem', whiteSpace: 'nowrap' }}>
                  <div className="spinner" />
                  <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '0.75rem', color: 'var(--accent)' }}>ANALIZA GEOMETRII…</span>
                </div>
              )}
            </div>
          )}
        ...
        {/* RIGHT — Config + Price */}
        <div className="wycena-panel-col">
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                <button onClick={() => setQuantity(q => Math.max(1, q - 1))}
                  style={{ width: '44px', height: '44px', background: 'var(--bg-3)', border: '1px solid var(--border)', color: 'var(--text)', cursor: 'pointer', fontSize: '1.2rem', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  −
                </button>
              ...
              {!volume ? (
                ...
                <>
                  <div style={{ marginBottom: '1.5rem' }}>
                    ...
                    <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 'clamp(1.75rem, 5vw, 2.5rem)', fontWeight: 700, color: quote ? 'var(--accent)' : 'var(--text-dim)', lineHeight: 1, transition: 'color 0.3s' }}>
                  ...
                  {quote && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem', marginBottom: '1.5rem', padding: '1rem', background: 'var(--bg-3)', borderLeft: '2px solid var(--border-light)' }}>
                      {[
                        { Icon: Package, label: 'Materiał', val: `${quote.breakdown.materialUsedGrams}g / ${quote.breakdown.materialCost.toFixed(2)} zł` },
                        { Icon: Clock,   label: 'Czas maszyny', val: `~${quote.breakdown.estimatedHours}h / ${quote.breakdown.timeCost.toFixed(2)} zł` },
                        { Icon: Layers,  label: 'Ilość', val: `× ${quantity} szt.` },
                      ].map(({ Icon, label, val }) => (
                        <div key={label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
                            <Icon size={12} style={{ color: 'var(--text-muted)' }} />
                            <span className="label-tag" style={{ margin: 0 }}>{label}</span>
                          </div>
                          <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '0.8rem' }}>{val}</span>
                        </div>
                      ))}
                    </div>
                  )}
